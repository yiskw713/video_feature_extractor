...
class SlowFast(nn.Module):
    ...
    def forward(self, input):
        # fast, lateral = self.FastPath(input[:, :, ::down_samp_rate, :, :])
        fast, lateral, motion_feats = self.FastPath(input)
        # slow = self.SlowPath(input[:, :, ::down_samp_rate * 8, :, :], lateral)
        slow, semantic_feats = self.SlowPath(input[:, :, ::8, :, :], lateral)

        x = torch.cat([slow, fast], dim=1)
        x = self.dp(x)
        x = self.fc(x)
        return x, semantic_feats, motion_feats
